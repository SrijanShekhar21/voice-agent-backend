...
class ChunkedStream(tts.ChunkedStream):
    ...
    async def _run(self, output_emitter: tts.AudioEmitter) -> None:
        # Detect language using langdetect on the first 10 words of input
        first_ten_words = " ".join(self._input_text.split()[:10])
        try:
            detected_lang = detect(first_ten_words)
        except Exception:
            detected_lang = "en"

        if detected_lang == "hi":
            self._opts.language = "hi-IN"
            self._opts.voice = "Kajal"
            self._opts.speech_engine = "neural"
        else:
            self._opts.language = "en-IN"
            self._opts.voice = "Kajal"
            self._opts.speech_engine = "neural"

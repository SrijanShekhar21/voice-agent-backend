...
class FrontDeskAgent(Agent):
    def __init__(self):
        super().__init__(
            instructions="You are a friendly receptionist. Ask the user how you can help—sales, technical support, or customer support—and route accordingly."
        )
    ...
    async def route_query(self, query: str):
        ...
        if "buy" in query or "price" in query or "product" in query:
            await self.session.say("I'm transferring you to Maya from sales.")

            # Start new session with Cartesia TTS
            new_session = AgentSession[DemoSessionData](
            ...
            await self.session.say("I'm transferring you to Maya from technical support.")
            return TechnicalSupportAgent()

        else:
            await self.session.say("I'm transferring you to Maya from customer support.")
